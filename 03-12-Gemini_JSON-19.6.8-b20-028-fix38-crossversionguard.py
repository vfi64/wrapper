...
class ConfigManager:
    ...
    def get_provider_model(self, provider: str = '') -> str:
        ...
        try:
            if provider == 'gemini':
                m2 = (self.config.get('model') or '').strip()
                if m2:
                    return m2
        except Exception:
            pass
...
async function buildUI() {
    ...
    if(btn) btn.style.display = (p === 'openrouter') ? 'block' : 'none';

    // Models list (optionally filter free models for OpenRouter)
    ...
    // Persist choice in localStorage (client-side only; no config write required)
    let freeOnly = false;
    try {
    ...
    const models = (freeOnly ? allModels.filter(m => (m || '').includes(':free')) : allModels);

    const modelOptions = models.map(m => ({value:m, label:`Model: ${m}`}));
    if(!modelOptions.length) modelOptions.push({value:'', label: isOpenRouter && freeOnly ? 'Model: (keine :free Modelle)' : 'Model: (no models available)'});
    // If current selection is not visible under filter, pick first visible model
    const desired = data.current_model || '';
    const visible = new Set((models || []).map(x => String(x)));
    const selected = (desired && visible.has(String(desired))) ? desired : (models[0] || '');
...
function toggleFreeOnly() {
    ...
    // Rebuild just the model dropdown based on cached models without a roundtrip
    const allModels = window._allModels || [];
    const models = v ? allModels.filter(m => (m || '').includes(':free')) : allModels;
    const modelOptions = (models || []).map(m => ({value:m, label:`Model: ${m}`}));
    if(!modelOptions.length) modelOptions.push({value:'', label: v ? 'Model: (keine :free Modelle)' : 'Model: (no models available)'});
    const current = document.getElementById('model').value || '';
    const visible = new Set((models || []).map(x => String(x)));
    const selected = (current && visible.has(String(current))) ? current : (models[0] || '');
    _setSelectOptions('model', modelOptions, selected);
    if(selected && selected !== current) {
        // Apply selection to backend so next call uses the visible model
...
class CSCRefiner:
    ...
    def _connect_api(self):
        ...
        if provider in ('openrouter', 'openai', 'openai_compat'):
            ...
            model = ''
            try:
                model = (getattr(cfg, 'get_provider_model', lambda _p: '')('openrouter') or '').strip()
            except Exception:
                model = ''
            if not model:
                try:
                    model = (cfg.get_model() or '').strip()
                except Exception:
                    model = ''
            ...
            if ok:
                try:
                    gov.log(f"Provider ready (OpenRouter) · model: {model or 'n/a'}")
                except Exception:
                    pass
                self.ready_status = {"status": True, "msg": f"Ready [openrouter:{model or 'n/a'}]", "filename": gov.current_filename}
            else:
                try:
                    gov.log("OpenRouter API key missing.")
                except Exception:
                    pass
                self.ready_status = {"status": False, "msg": "OpenRouter API key missing (set OPENROUTER_API_KEY or config providers.openrouter.api_key_plain)!"}
            return
    ...
    def _llm_call(self, user_text: str, *, reason: str = 'chat', model_override: str = ''):
        ...
        if provider in ('openrouter', 'openai', 'openai_compat'):
            ...
            try:
                ...
                client = None
            if client is None:
                raise RuntimeError('OpenRouter client not configured (missing API key?)')
            ...
            try:
                ...
                fallback = ''
            model = (model_override or self._provider_model('openrouter', fallback_model=fallback) or '').strip()
            if not model:
                # Optional: auto-pick first model from cached /models list (best-effort)
                try:
                    models, _meta = pr.get_openrouter_models_cached(force_refresh=False) if pr is not None else ([], {})
                    if models:
            ...
            try:
                ...
                pconf = provs.get('openrouter') if isinstance(provs, dict) else {}
...
# Auto-reset: Dynamic one-shot (canonical JSON)
    ...
    def refresh_models(self):
        """Refresh provider model list cache (OpenRouter only)."""
        try:
            pr = getattr(self, 'provider_router', None)
            ...
            models, meta = pr.get_openrouter_models_cached(force_refresh=True) if pr is not None else ([], {})
            # UI notice
            try:
                if self.main_win:
    ...
    def get_ui(self):
        ...
        try:
            ...
            data['providers'] = ['gemini', 'openrouter']
        except Exception:
            pass
        ...
        try:
            ...
            if curp == 'gemini':
                ...
                if pr is not None and hasattr(pr, 'get_openrouter_models_cached'):
                    ...
                    models = []
            # UI: keep the dropdown usable even with many models
            try:
                # de-dup + alpha sort (case-insensitive)
                models = sorted(list(dict.fromkeys(models or [])), key=lambda s: str(s).lower())
            except Exception:
                pass
...
class ProviderRouter:
    ...
    def build_openrouter_client(self):
        ...
        try:
            provs = (self.cfg.config or {}).get('providers') or {}
            pconf = (provs.get('openrouter') or {}) if isinstance(provs, dict) else {}
