    ...
    if os.path.exists(key_path):
        try:
            with open(key_path, "r", encoding="utf-8") as f:
                ...
                if key:
                    print(f"[System] API key loaded from {KEYS_FILENAME}.")
                    return key
...
class GovernanceRuntimeState:
    ...
    last_qc_deltas: dict = field(default_factory=dict)
    user_turns: int = 0
...
function changeModel() {
    ...
    window.pywebview.api.set_model(model);
}
...
class CSCRefiner:
    ...
    def _recreate_chat_session(self, with_governance: bool, reason: str = "") -> bool:
        ...
        - with_governance=True: uses self._get_governed_system_instruction() (canonical JSON + runtime state)
    ...
    def _apply_csc_strict(self, raw_response: str, *, user_raw: str, is_command: bool):
        ...
        try:
            ...
            if is_command:
                return markdown.markdown(raw_response, extensions=['extra', 'codehilite']), None
    ...
    def _execute_legacy_command(self, cmd: str):
        ...
        # 4. SCI Control
            ...
            if cur >= max_depth:
                # hard guard; do nothing
                return

            # Snapshot parent SCI variant so we can deterministically return after the next answer
            try:
                self.gov_state.sci_recursion_parent_variant = getattr(self.gov_state, 'sci_variant', '') or ''
                self.gov_state.sci_recursion_depth = cur + 1
                self.gov_state.sci_recursion_one_shot = True
            except Exception:
                pass

            # Ensure SCI is at least active (Variant A fallback) so recursion has a defined trace context
            try:
                if not bool(getattr(self.gov_state, 'sci_active', False)):
                    self.gov_state.sci_active = True
                if not (getattr(self.gov_state, 'sci_variant', '') or '').strip():
    ...
    def _handle_sci_selection(self, letter: str):
        ...
        # Refresh model session with updated runtime state
        try:
            self._recreate_chat_session(with_governance=True, reason=f"SCI select {char}")
    ...
    def _get_governed_system_instruction(self):
        """Build system instruction robustly from canonical JSON + current runtime state.
        Fixes: TypeErrors from None/list/dict concatenation; ensures runtime state (profile/overlay/SCI/color/lang)
        is actually visible to the model.
        """

        def _to_text(x) -> str:
            """Safe stringify. Lists become joined lines; dicts become JSON."""
            try:
                if x is None:
                    return ""
                if isinstance(x, str):
                ...
                if isinstance(x, list):
                    ...
                    return "\n".join(parts)
                if isinstance(x, dict):
                    return json.dumps(x, ensure_ascii=False, indent=0, sort_keys=True)
                return str(x)
            except Exception:
                return ""
        ...
        # Base instruction (may be None/list/etc in JSON)
        sys_instr = _to_text((data.get("global_defaults", {}) or {}).get("system_instruction_base", "")).strip()

        # Always include canonical JSON (source of truth) so the model can validate tokens/behaviors
        raw_rules = ""
        try:
            raw_rules = getattr(gov_obj, "raw_json", "") or ""
        except Exception:
            raw_rules = ""
        if raw_rules.strip():
            sys_instr = (sys_instr + "\n\n" + "GOVERNANCE RULES (CANONICAL JSON):\n" + raw_rules.strip()).strip()

        # Profile addendum (optional)
        prof_name = getattr(getattr(self, "gov_state", None), "active_profile", "Standard") or "Standard"
        prof_data = (data.get("profiles", {}) or {}).get(prof_name, {}) if isinstance(data.get("profiles", {}), dict) else {}
        add_profile = _to_text((prof_data or {}).get("system_instruction_add", "")).strip()
        if add_profile:
            sys_instr = (sys_instr + "\n\n" + add_profile).strip()

        # Overlay (Strict / Explore)
        overlay = getattr(getattr(self, "gov_state", None), "overlay", "") or ""
        if overlay == "Explore":
            sys_instr = (sys_instr + "\n\n" + "MODE: EXPLORE. Encourage hypotheses. Be creative. Verification logic is relaxed.").strip()
        elif overlay == "Strict":
            sys_instr = (sys_instr + "\n\n" + "MODE: STRICT. Demand evidence. Do not speculate. High verification standards.").strip()

        # SCI (derived ONLY from JSON)
        try:
            if bool(getattr(getattr(self, "gov_state", None), "sci_active", False)):
                letter = (getattr(getattr(self, "gov_state", None), "sci_variant", "") or "").strip().upper()

                sci = data.get("sci", {}) or {}
                variant_menu = (sci.get("variant_menu", {}) or {})
                variants = (variant_menu.get("variants", {}) or {}) if isinstance(variant_menu, dict) else {}
                vdef = (variants.get(letter, {}) or {}) if isinstance(variants, dict) else {}

                vname = _to_text(vdef.get("name", "")).strip()
                vfocus = _to_text(vdef.get("focus", "")).strip()

                steps = []
                if isinstance(vdef.get("trace_steps"), list):
                    steps = [str(s).strip() for s in vdef.get("trace_steps") if str(s).strip()]

                if not steps:
                    maps_to = vdef.get("maps_to")
                    mode_key = None
                    if isinstance(maps_to, dict) and maps_to.get("type") == "sci_mode":
                        mode_key = maps_to.get("value")
                    modes = (sci.get("modes", {}) or {})
                    if mode_key and isinstance(modes.get(mode_key, {}), dict):
                        steps = modes.get(mode_key, {}).get("steps", []) or []
                    if not steps and isinstance(modes.get("SCI", {}), dict):
                        steps = modes.get("SCI", {}).get("steps", []) or []

                steps = [str(s).strip() for s in (steps or []) if str(s).strip()]
                steps_txt = ""
                if steps:
                    steps_txt = "Steps: " + " → ".join(steps)

                line = f"SCI ACTIVE: Variant {letter}"
                if vname:
                    line += f" · {vname}"
                if vfocus:
                    line += f" · {vfocus}"
        ...
        # Evidence Linker + Color mode (derived ONLY from JSON)
        try:
            prof = prof_name
            ...
            el = (data.get("global_defaults", {}) or {}).get("evidence_linker", {}) or {}

            # Canonical strings from JSON (Source of Truth)
            color_on_fn = _to_text(((data.get("commands", {}) or {}).get("color_control", {}) or {}).get("Color on", {}) or {}).get("function", "").strip()
            color_off_fn = _to_text(((data.get("commands", {}) or {}).get("color_control", {}) or {}).get("Color off", {}) or {}).get("function", "").strip()
            activation_notes = _to_text((el.get("activation", {}) or {}).get("notes", "")).strip()
            tag_rule = _to_text(((el.get("rendering", {}) or {}).get("tagging_requirement", {}) or {}).get("rule", "")).strip()
            placement_example = _to_text(((el.get("rendering", {}) or {}).get("placement", {}) or {}).get("example", "")).strip()
            prov_note = _to_text(((el.get("rendering", {}) or {}).get("provenance_suffixes", {}) or {}).get("note", "")).strip()

            tag_prefix = (((el.get("rendering", {}) or {}).get("representation_templates", {}) or {}).get("tag_prefix", {}) or {})
            if not isinstance(tag_prefix, dict):
                tag_prefix = {}
            tp_green = _to_text(tag_prefix.get("GREEN", "")).strip()
            tp_yellow = _to_text(tag_prefix.get("YELLOW", "")).strip()
            tp_red = _to_text(tag_prefix.get("RED", "")).strip()

            # Compact, explicit instruction so the model actually emits tags (renderer depends on them)
            block_lines = []
            block_lines.append(f"EVIDENCE LINKER COLOR MODE: {str(color).upper()}")

            if str(color).lower() == "on" and color_on_fn:
                block_lines.append(color_on_fn)
            if str(color).lower() != "on" and color_off_fn:
                block_lines.append(color_off_fn)

            if activation_notes:
                block_lines.append(activation_notes)
            if tag_rule:
                block_lines.append(tag_rule)

            # Choose a deterministic representation for the GUI renderer
            # (Uses JSON templates verbatim; does not invent tokens.)
            if tp_green or tp_yellow or tp_red:
                block_lines.append("Representation: tag_prefix")
                if tp_green:
                    block_lines.append(f"GREEN prefix: {tp_green}")
                if tp_yellow:
                    block_lines.append(f"YELLOW prefix: {tp_yellow}")
                if tp_red:
                    block_lines.append(f"RED prefix: {tp_red}")

            if placement_example:
                block_lines.append(f"Example: {placement_example}")
            if prov_note:
                block_lines.append(prov_note)

            sys_instr = (sys_instr + "\n\n" + "\n".join([l for l in block_lines if l])).strip()
        except Exception:
            pass

        # SCI Recursive (canonical JSON: sci.recursive_sci)
        try:
            rs = (data.get('sci') or {}).get('recursive_sci') or {}
            depth = int(getattr(getattr(self, 'gov_state', None), 'sci_recursion_depth', 0) or 0)
            one_shot = bool(getattr(getattr(self, 'gov_state', None), 'sci_recursion_one_shot', False))
            scope = (getattr(getattr(self, 'gov_state', None), 'sci_recursion_scope', '') or '').strip()
            if (depth > 0) and (one_shot or scope):
                title_tpl = _to_text(rs.get('nested_block_title_template', 'SCI Recursion (depth {depth})')).strip()
                scope_tpl = _to_text(rs.get('scope_line_template', 'Scope: {scope}')).strip()
                try:
                    title = title_tpl.format(depth=depth)
                except Exception:
                    title = f"SCI Recursion (depth {depth})"
                try:
                    scope_line = scope_tpl.format(scope=scope)
                except Exception:
                    scope_line = f"Scope: {scope}"
                # Token budget guidance (best-effort): communicate an explicit per-level budget.
                try:
                    max_tokens = int(rs.get('max_tokens_per_level', 350) or 350)
                except Exception:
                    max_tokens = 350
                try:
                    adjusted_tokens = max(64, int(max_tokens // max(1, (depth + 1))))
                except Exception:
                ...
                sys_instr = (
                    ...
                    + "\n"
                    + scope_line
                    + f"\nMax tokens for this level: {adjusted_tokens}"
                    + "\nInstruction: Answer ONLY the scoped sub-question as a nested SCI Trace. Do NOT include the parent trace."
                    + "\nAfter completing the nested trace, output 'END RECURSION' and stop."
                ).strip()
        except Exception:
            pass

        # Dynamic one-shot prompting (canonical JSON: global_defaults.dynamic_prompting)
        try:
            if bool(getattr(getattr(self, 'gov_state', None), 'dynamic_one_shot_active', False)):
                dp = ((data.get('global_defaults') or {}).get('dynamic_prompting') or {})
                one = (dp.get('one_shot_override') or {}) if isinstance(dp, dict) else {}
                if bool(one.get('enabled', False)):
                    trigger = (dp.get('trigger') or {}) if isinstance(dp, dict) else {}
                    try:
                        thr = int(trigger.get('delta_abs_threshold', 2) or 2)
                    except Exception:
                        thr = 2
                    actions = (dp.get('actions') or {}) if isinstance(dp, dict) else {}
                    deltas = getattr(getattr(self, 'gov_state', None), 'last_qc_deltas', {}) or {}

                    dyn_lines = ["DYNAMIC: one-shot override is ACTIVE for the next answer only."]
                    if isinstance(deltas, dict) and isinstance(actions, dict):
                        for dim, delta in deltas.items():
                            ...
                            if abs(d) < thr:
                                continue
                            act = actions.get(dim, {}) or {}
                            if not isinstance(act, dict):
                                continue
                            instr = _to_text(act.get('delta_negative' if d < 0 else 'delta_positive', '')).strip()
                            if instr:
                                dyn_lines.append(instr)

                    sys_instr = (sys_instr + "\n\n" + "\n".join(dyn_lines)).strip()
        except Exception:
            pass

        # Language policy hint to model
        sys_instr = (sys_instr + "\n\nIMPORTANT: Keep all scaffolding (headers/labels/SCI step names/QC-matrix labels/command outputs) in English. The wrapper may request a different language for the final answer content via [OUTPUT LANGUAGE].").strip()
    ...
    def ask(self, txt):
        ...
        try:
            ...
            if route["kind"] == "command":
                ...
                # After state change: refresh model session so runtime state is applied
                try:
                    self._recreate_chat_session(with_governance=True, reason=f"cmd:{cmd}")
                ...
                if bool(getattr(self.gov_state, 'sci_pending', False)) and not route.get('is_sci_selection'):
                    ...
                    if self.gov_state.sci_pending_turns >= max_turns:
                        ...
                        try:
                            ...
                            pass
                        # Ensure model sees the new state
                        try:
                            self._recreate_chat_session(with_governance=True, reason="SCI pending timeout -> A")
                            self._last_session_stamp = _session_stamp()
                        except Exception:
                            pass
            ...
            try:
                if not getattr(self, "chat_session", None):
                    ...
                    if last != cur:
                        self._recreate_chat_session(with_governance=True, reason="state_changed")
            ...
            user_for_model = self._apply_output_prefs_to_user_message(raw_txt)
...
# Auto-reset: Dynamic one-shot (canonical JSON)
            try:
                if bool(dynamic_was_active):
                    ...
                    # refresh session so the marker/instruction disappears deterministically
                    try:
                        self._recreate_chat_session(with_governance=True, reason="Dynamic one-shot auto-reset")
                        self._last_session_stamp = _session_stamp()
                    except Exception:
                        pass
            except Exception:
            ...
            try:
                if bool(getattr(self.gov_state, 'sci_recursion_one_shot', False)):
                    ...
                    # refresh session to return to parent trace deterministically
                    try:
                        self._recreate_chat_session(with_governance=True, reason="SCI recurse return")
                        self._last_session_stamp = _session_stamp()
                    except Exception:
                        pass
            except Exception:
...
# ----------------------------
for _name in ("_render_error_html", "_safe_html", "_handle_command_deterministic", "_as_dict", "_as_list", "_safe_get", "_render_error_fallback"):
